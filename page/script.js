...
d3.json(`./data/${switch_to_city}/data.json`).then(function (json) {
      ...
      if (rotate_degree <= 180) {
        if (i > 1) {
          if (d.year == prevData["year"] && d.year == prevData2["year"]) {
            //three dots with the same year
            console.log(`same year2:${prevData2["year"]}`);
            return (
              (start_dot_originalX + 20) * Math.cos(toRadians(rotate_degree))
            );
          } else if (d.year == prevData["year"]) {
            //two dots with the same year
            return (
              (start_dot_originalX + 10) * Math.cos(toRadians(rotate_degree))
            );
          } else {
            //others
        ...
        if (i > 0) {
          if (d.year == prevData["year"] && d.year == prevData2["year"]) {
            return (
              -1 *
              (start_dot_originalX + 20) *
              Math.cos(toRadians(rotate_degree - 180))
            );
          } else if (d.year == prevData["year"]) {
            //two dots with the same year
            return (
              -1 *
              (start_dot_originalX + 10) *
              Math.cos(toRadians(rotate_degree - 180))
            );
          } else {
            return (
          ...
          return (
            -1 * start_dot_originalX * Math.cos(toRadians(rotate_degree - 180))
          );
        }
      }
    })
    .attr("cy", (d, i) => {
      ...
      if (rotate_degree <= 180) {
        if (i > 1) {
          if (d.year == prevData["year"] && d.year == prevData2["year"]) {
            return (
              (start_dot_originalX + 20) * Math.sin(toRadians(rotate_degree)) +
              start_dot_originalY
            );
          } else if (d.year == prevData["year"]) {
            return (
              (start_dot_originalX + 10) * Math.sin(toRadians(rotate_degree)) +
              start_dot_originalY
            );
          } else {
            return (
              start_dot_originalX * Math.sin(toRadians(rotate_degree)) +
              start_dot_originalY
            );
          }
        } else if (i > 0) {
        ...
        if (i > 0) {
          if (d.year == prevData["year"] && d.year == prevData2["year"]) {
            return (
              start_dot_originalY -
              (start_dot_originalX + 20) *
                Math.sin(toRadians(rotate_degree - 180))
            );
          }
          if (d.year == prevData["year"]) {
            return (
            ...
            return (
              start_dot_originalY -
              start_dot_originalX * Math.sin(toRadians(rotate_degree - 180))
            );
          }
        } else {
          return (
  ...
  var labels = timeline
    ...
    .attr("transform", (d) => {
      ...
      if (rotate_degree <= 180) {
        ...
        trans_x =
          -1 * (wheel_radius - 5) * Math.cos(toRadians(rotate_degree - 180));
      }
      //trans_y
      if (rotate_degree <= 180) {
        trans_y =
          (wheel_radius - 5) * Math.sin(toRadians(rotate_degree)) +
          start_dot_originalY;
      } else if (rotate_degree <= 270) {
        trans_y =
          start_dot_originalY -
          (wheel_radius - 5) * Math.sin(toRadians(rotate_degree - 180));
      }
  ...
  var text_item = svg
    ...
    .style(
      "transform",
      `translate3d(20px, ${text_item_height}px,0) rotate(90deg)`
    )
    .style("opacity", 0);
  ...
  wheel
    ...
    .style(
      "transform",
      `translate3d(-${wheel_radius}px, 50px,0) rotate(90deg)`
    );
  //dots and labels
  timeline
  ...
  fisrt_content_top
    ...
    .style(
      "transform",
      `translate3d(20px,${text_item_height}px,0) rotate(0deg)`
    )
    .style("opacity", 1);
  ...
  function rotation_def(index, up_down, data) {
    ...
    if (up_down == "up") {
      ...
      last_word_text_top
        ...
        .style(
          "transform",
          `translate3d(20px, ${text_item_height}px,0) rotate(-180deg)`
        )
        .style("opacity", 0);
    ...
    timeline
      .style("transform-origin", `0px ${start_dot_originalY}px`)
      .style("transform", `translate3d(0,0,0) rotate(${sumAngle}deg)`);
    currentWheel
      .style("transform-origin", `${wheel_radius}px ${wheel_radius}px`)
      .style(
        "transform",
        `translate3d(-${wheel_radius}px, 50px,0) rotate(${wheel_sumAngle}deg)`
      );
        ...
        success: function (response) {
          $.each(response, function (i, e) {
            ...
            if (switch_to_city === "Durham" || switch_to_city === "Essex") {
              $("#about-body-1").append(
                `<a href="${e.link}" target="_blank">plans to mitigate and adapt to the effects of climate change.</a>`
              );
            }
            //Trustees-clickable text
            if (switch_to_city === "Trustees") {
              $("#about-body-1").append(
                `<a href="${e.link}" target="_blank">The Trustees has responded</a> <span>${e.body1a}</span>`
              );
            }
            ...
            if (switch_to_city === "Saugus") {
              $("#about-body-1").append(
                `<a href="${e.link}" target="_blank">Saugus Iron Works NHS</a> <span>${e.body1a}</span>`
              );
            }
            //Arlington-clickable text
            if (switch_to_city === "Arlington") {
            ...
            if (switch_to_city === "Newburyport") {
              $("#about-body-2").append(
                `<a href="${e.link}" target="_blank">planned projects to deal with them.</a> <span>${e.body2a}</span>`
              );
            ...
            if (switch_to_city === "Arlington") {
              ...
              $("#about-body-3").append(
                `<a href="${e.link3a}" target="_blank">Remembrance of Climate Futures,</a> <span>${e.body3a}</span>`
              );
            }
